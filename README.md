...
- **기상청 실시간 특보** — 알림 탭은 데모 데이터가 아니라 **기상청(KMA) 특보 현황**으로
  현재 발효 중인 실제 특보(호우주의보 등)만 표시. **백엔드 프록시**(`/api/weather/warning-bulletin`)가
  기상청을 호출하므로 **웹/모바일 모두** 동작(웹 CORS·키 노출 없음). 통보문 원문을 받아
  사용자 지역 키워드로 매칭하며, 발효 특보가 없으면 "발효 특보 없음"을 표시
...
**클라이언트(.env)** — 이 앱(프론트엔드)에서 쓰는 변수:

| 변수 | 용도 | 필수 |
|---|---|---|
| `KAKAO_JS_KEY` | 웹 카카오 지도 + 주소 위경도 지오코딩 (공개 클라이언트 키) | 웹 지도 사용 시 |
| `KAKAO_NATIVE_KEY` | 네이티브 카카오 SDK (현재 빌드 미사용) | ✗ |
| `FLOOD_API_BASE_URL` | 예측/기상 API 베이스 URL 오버라이드 | ✗ |

> ℹ️ **기상청 키(`KMA_SERVICE_KEY`)는 더 이상 클라이언트에 넣지 않습니다.** 캘린더 강수량·알림
> 특보·주간 예보는 모두 **백엔드 프록시**가 기상청을 호출하므로(웹 CORS·키 노출 회피),
> 키는 **백엔드(Ready-Flow) Vercel 환경변수**에만 둡니다. 아래 *백엔드 환경변수* 참고.

**백엔드(Ready-Flow / `doubled_seven`) — Vercel 환경변수:**

| 변수 | 용도 | 필수 |
|---|---|---|
| `KMA_SERVICE_KEY` | 기상청 ASOS 일자료·특보·단기예보 서버 호출 | 기상 기능 사용 시 |

> data.go.kr 에서 **세 서비스 모두** 같은 키로 활용신청해야 합니다(각각 별도 신청):
> ① 기상특보 조회서비스 ② 지상(ASOS) 일자료 조회서비스 ③ 단기예보 조회서비스(VilageFcstInfoService_2.0).
> 미신청 서비스는 해당 기능만 `503` 안내를 표시하고 나머지는 정상 동작합니다.
